...
class StoreRepositories(StoreInterface):
    ...
    async def get_store_by_store_code(self, conn: AsyncConnection, store_code: str) -> RowMapping | None:
        stmt = select(store).where(eq(store.c.store_code, store_code)).exists()
