...
from sqlalchemy import and_, cast, func, Null, RowMapping, select
            ...
            func.coalesce(
                func.jsonb_agg(
                    func.jsonb_build_object(
                        "user_uuid", user.c.uuid,
                        "full_name", user.c.full_name,
                    )
                ).filter((user.c.uuid.isnot(None)) & (user.c.deleted_at.is_(None))), cast("[]", JSONB),
            ).label("capsters"),
        ).join(
            capster, eq(store.c.uuid, capster.c.store_uuid), isouter=True
        ).join(

            user, eq(user.c.uuid, capster.c.user_uuid), isouter=True
        ).where(
            eq(store.c.uuid, store_uuid)
