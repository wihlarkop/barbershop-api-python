...
class UserInterface(Protocol):
    ...
    async def update_user(self, conn: AsyncConnection): ...

    async def activate_user(self, conn: AsyncConnection): ...


class ProductInterface(Protocol):
    async def get_products(self, conn: AsyncConnection) -> Sequence[RowMapping]: ...

    async def get_product_by_uuid(self, product_uuid: UUID, conn: AsyncConnection): ...
