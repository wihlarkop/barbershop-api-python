...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(name="verification_at", type_=sa.TIMESTAMP(timezone=True), default=datetime.now(tz=timezone)),

        sa.ForeignKeyConstraint(name="fk_verification_user", columns=["user_uuid"], refcolumns=["user.uuid"]),
        sa.ForeignKeyConstraint(name="fk_verification_store", columns=["store_uuid"], refcolumns=["store.uuid"]),
    )
